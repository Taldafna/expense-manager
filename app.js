...
function renderMonthDashboard() {
    ...
    const prevForecast = dataManager.getForecast(app.currentUser, prevYearStr, prevMonthStr);
    const prevSpent = dataManager.getTotalSpent(app.currentUser, prevYearStr, prevMonthStr);
    const prevBudget = Object.values(prevForecast.budgets).reduce((sum, v) => sum + (v || 0), 0);

    if (prevSpent > prevBudget && prevBudget > 0) {
        const overrun = prevSpent - prevBudget;
        const alertDiv = document.createElement('div');
        alertDiv.className = 'overrun-alert';
        alertDiv.innerHTML = `
            <div class="icon">🚨</div>
            <div class="overrun-alert-content">
                <span class="overrun-alert-title">חריגה מחודש שעבר!</span>
                <span class="overrun-alert-desc">בחודש ${MONTH_NAMES[prevMonth - 1]} חרגת ב-${formatCurrency(overrun)}. החודש מומלץ לחסוך ${formatCurrency(overrun)} יותר כדי לאזן!</span>
            </div>
        `;
    ...
    categories.forEach(category => {
        const budget = forecast.budgets[category] || 0;
        const categorySpent = spent[category] || 0;
        const categoryRemaining = budget - categorySpent;
        const percent = budget > 0 ? Math.min((categorySpent / budget) * 100, 100) : 0;
        const overBudget = categorySpent > budget && budget > 0;
        ...
        let alertClass = '';
        if (budget > 0) {
            const p = (categorySpent / budget) * 100;
        ...
        item.innerHTML = `
            <div class="category-header">
                ...
                <span class="category-amounts">
                    <span>${formatCurrency(categorySpent)}</span> / ${formatCurrency(budget)}
